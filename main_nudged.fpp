...
! add_nudge_forcing, scramble_phases_above).
!
! CONTINUOUS NUDGING VARIANT (main_nudged.fpp)
! ---------------------------------------------
! main.fpp resynchronizes each ensemble member's large scales
! (k <= kndg) with the reference by hard-overwriting field_nxt
! every single timestep via replace_scales. Per
! energy_excess_investigation.md, this was found to inject a
! phase discontinuity at k ~ kndg every step, producing a
! localized excess-energy bump just above kndg that accounts for
! essentially all of the members' spurious energy excess over
! the reference.
!
! This variant replaces the post-step hard overwrite with a
! continuous linear relaxation ("nudging") source term added to
! the member's RHS during the step:
!
!     du_member/dt = RHS_NS(u_member) + force
!                    - lambda*(u_member(k) - u_ref(k))   for k <= kndg
!
! implemented by adding lambda*(ref-mem) into a per-member copy
! of the shared forcing array (add_nudge_forcing) before calling
! stepper%gstep, so the relaxation is integrated consistently
! with the rest of the dynamics instead of teleporting the state
! after the fact. The k <= kndg cutoff itself is unchanged from
! replace_scales, so this isolates the temporal-discontinuity fix
! from any change to the spectral cutoff shape.
!
! lambda (namelist parameter nudge_lambda) is a fixed, externally
! chosen value -- not computed by this code. Pick it from the
! large-scale eddy turnover time at kndg, e.g.
! lambda ~ 1/tau_eddy(kndg) = sqrt(kndg^3 * E(kndg)), estimated
! offline from the reference spectrum.
!
! Initial conditions (stat=0, fresh run): rather than giving each
! member its own independent IC and then hard-syncing k<=kndg via
! replace_scales, each member's IC is built as a full copy of the
! reference field with the phases of its k>kndg modes scrambled
! (scramble_phases_above) -- same energy spectrum as the reference
! everywhere, but the un-nudged scales start as an independent
! (decorrelated) realization instead of an exact copy. The nudged
! shells k<=kndg start as an exact copy of the reference, same as
! replace_scales produced. For restarts (stat/=0), the original
! behavior is kept: each member reads its own checkpoint via its
! iclist, then replace_scales re-syncs k<=kndg from the restored
! reference.
!=================================================================
      MODULE synchro_mod
...
! Type encapsulating all state for a single nudged simulation member
      TYPE :: GNudgedSim
         ...
         TYPE(GStateComp),    ALLOCATABLE :: force_nudged(:)
...
!
! In this nudging variant, only used once to synchronize the
! initial conditions of the ensemble members with the reference
! before the time loop starts -- the per-step call is replaced
! by add_nudge_forcing below.
!=================================================================
...
!=================================================================
! SUBROUTINE: add_nudge_forcing
!
! Adds a linear relaxation ("nudging") source term to frc, for
! all wavenumber shells k <= kcut:
!
!     frc(k) <- frc(k) + lambda*(ref(k) - mem(k))
!
! Shells k > kcut are left unchanged. Meant to be applied to a
! per-member copy of the shared 'force' array before it is
! passed to stepper%gstep, so the large-scale relaxation toward
! the reference is integrated as part of the RHS every step,
! rather than applied as a hard overwrite after the step.
!=================================================================
      SUBROUTINE add_nudge_forcing(ref, mem, kcut, lambda, frc)
          ...
          TYPE(GStateComp), INTENT(IN)    :: ref(:), mem(:)
          INTEGER,          INTENT(IN)    :: kcut
          REAL(KIND=GP),    INTENT(IN)    :: lambda
          TYPE(GStateComp), INTENT(INOUT) :: frc(:)
          ...
          DO ic = 1, SIZE(ref)
              DO i = ista, iend
                  DO j = 1, ny
                      DO k = 1, nz
                          IF (int(sqrt(kk2(k,j,i))/Dkk+0.5_GP) .le. kcut) THEN
                              frc(ic)%ccomp(k,j,i) = frc(ic)%ccomp(k,j,i) + &
                                  lambda*(ref(ic)%ccomp(k,j,i) - mem(ic)%ccomp(k,j,i))
                          ENDIF
                      END DO
                  END DO
              END DO
...
!=================================================================
! SUBROUTINE: scramble_phases_above
!
! Randomizes the phase of every mode in 'fld' at wavenumber
! shells k > kcut, leaving shells k <= kcut untouched and
! preserving the energy at every mode (only the phase is
! randomized, |fld(k)| is unchanged). Used to turn a full copy
! of the reference field into a decorrelated-but-same-spectrum
! initial condition for an ensemble member's un-nudged scales.
!
! Follows the same reality-preserving pattern used by GHOST's
! own random_v initial condition (ic_velocity.f90): the kx=0
! plane (i=1) needs conjugate-symmetric phase pairs so the
! physical-space field stays real, while every other mode
! (i>1) can get an independent random phase.
!=================================================================
      SUBROUTINE scramble_phases_above(fld, kcut)
          ...
          TYPE(GStateComp), INTENT(INOUT) :: fld(:)
          INTEGER,          INTENT(IN)    :: kcut
          ...
          DO ic = 1, SIZE(fld)
             IF (ista.eq.1) THEN
                ! kx=0 plane: conjugate-symmetric phase pairs (j-axis, k=1)
                DO j = 2, ny/2+1
                   IF (int(sqrt(kk2(1,j,1))/Dkk+0.5_GP) .gt. kcut) THEN
                      phase = 2*pi*randu(seed)
                      fld(ic)%ccomp(1,j,1)      = fld(ic)%ccomp(1,j,1)* &
                                                   (COS(phase)+im*SIN(phase))
                      fld(ic)%ccomp(1,ny-j+2,1) = conjg(fld(ic)%ccomp(1,j,1))
                   ENDIF
                END DO
                ! kx=0 plane: conjugate-symmetric phase pairs (k-axis, j=1)
                DO k = 2, nz/2+1
                   IF (int(sqrt(kk2(k,1,1))/Dkk+0.5_GP) .gt. kcut) THEN
                      phase = 2*pi*randu(seed)
                      fld(ic)%ccomp(k,1,1)      = fld(ic)%ccomp(k,1,1)* &
                                                   (COS(phase)+im*SIN(phase))
                      fld(ic)%ccomp(nz-k+2,1,1) = conjg(fld(ic)%ccomp(k,1,1))
                   ENDIF
                END DO
                ! kx=0 plane: conjugate-symmetric phase pairs (mixed j,k)
                DO j = 2, ny
                   DO k = 2, nz/2+1
                      IF (int(sqrt(kk2(k,j,1))/Dkk+0.5_GP) .gt. kcut) THEN
                         phase = 2*pi*randu(seed)
                         fld(ic)%ccomp(k,j,1) = fld(ic)%ccomp(k,j,1)* &
                                                 (COS(phase)+im*SIN(phase))
                         fld(ic)%ccomp(nz-k+2,ny-j+2,1) = conjg(fld(ic)%ccomp(k,j,1))
                      ENDIF
                   END DO
                END DO
                ! remaining kx>0 modes on this rank: independent phases
                DO i = 2, iend
                   DO j = 1, ny
                      DO k = 1, nz
                         IF (int(sqrt(kk2(k,j,i))/Dkk+0.5_GP) .gt. kcut) THEN
                            phase = 2*pi*randu(seed)
                            fld(ic)%ccomp(k,j,i) = fld(ic)%ccomp(k,j,i)* &
                                                    (COS(phase)+im*SIN(phase))
                         ENDIF
                      END DO
                   END DO
                END DO
             ELSE
                ! this rank holds only kx>0 modes: independent phases
                DO i = ista, iend
                   DO j = 1, ny
                      DO k = 1, nz
                         IF (int(sqrt(kk2(k,j,i))/Dkk+0.5_GP) .gt. kcut) THEN
      ...
      END SUBROUTINE scramble_phases_above
...
! Synchro ------------------------------------------
      ...
      NAMELIST / ensembleparams / num_realizations, kndg, nudge_lambda
! Synchro ------------------------------------------
...
! Synchro ------------------------------------------
      ...
      ! reference spectrum), not computed by this code.
      nudge_lambda = 1.0_GP
      IF (myrank.eq.0) THEN
...
! Synchro ------------------------------------------
      ...
      DO ir = 1, SIZE(ensemble)
         ...
         CALL GState_alloc(ensemble(ir)%force_nudged, ensemble(ir)%num_components)
         BLOCK
...
! Initialize states and stepper
      ...
      WRITE(outlabel,'(I3.3)') ir
      IF (stat.eq.0) THEN
         ! Fresh run: build this member's IC as a full copy of the
      ...
      ELSE
         ! Restart: each member reads back its own checkpoint.
...
! On restart, re-sync k<=kndg from the restored reference, same as
! main.fpp always did (not needed on a fresh run: the copy+scramble
! above already leaves k<=kndg an exact match to the reference).
      IF (stat.ne.0) THEN
         DO ir = 1, SIZE(ensemble)
      ...
      IF (myrank.eq.0) THEN
         PRINT *, 'Nudging: lambda = ', nudge_lambda, &
                  '  (tau = ', 1.0_GP/nudge_lambda, ')'
      ENDIF
! Synchro ------------------------------------------
...
! Synchro ----------------------------------------------------------
! Step ensemble members with a continuous nudging forcing term
! (lambda*(ref-mem) added to a per-member copy of the shared
! forcing, for k <= kndg) instead of a post-step replace_scales.
         DO ir = 1, SIZE(ensemble)
            ensemble(ir)%field = ensemble(ir)%field_nxt
            ensemble(ir)%force_nudged = force
            CALL add_nudge_forcing(field, ensemble(ir)%field, kndg, &
                                   nudge_lambda, ensemble(ir)%force_nudged)
            CALL ensemble(ir)%stepper%gstep(time, ensemble(ir)%field, &
                                            ensemble(ir)%force_nudged, dt, &
